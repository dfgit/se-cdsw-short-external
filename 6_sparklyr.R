## Load libraries
# Based on Jordan's version, not Matt B's
# installs packages for SparklyR used in 4_sparklyr.R, which can take awhile. 
# 4_sparklyr.R will install libs if they're not already isntalled 


# install / load packages
source("UTILS/env_utils.R")
pkgs <- c("sparklyr", "dplyr", "ggplot2","maps", "geosphere", "DBI")
installLoadPkgs(pkgs)

app_name <- set_app_name("Sparklyr Flights")
sprintf("Spark History App Name: %s", app_name)

### Configure Spark context
sc <- spark_connect(master="yarn-client", app_name=app_name)

dbSendQuery(sc,"CREATE DATABASE IF NOT EXISTS flights")
# tbl_change_db(sc,"flights")
dbSendQuery(sc, "USE flights")

dbSendQuery(sc,"CREATE EXTERNAL TABLE IF NOT EXISTS airports_str 
  ( iata STRING,airport STRING, city STRING,state STRING, country STRING,    latitude DOUBLE,    longitude DOUBLE ) 
  ROW FORMAT  SERDE 'org.apache.hadoop.hive.serde2.OpenCSVSerde' 
  STORED AS TEXTFILE LOCATION '/tmp/airports/' TBLPROPERTIES('skip.header.line.count'='1')")

dbSendQuery(sc,"create table if not exists flights.airports as select iata, city, state, country, 
                cast(latitude as double), cast (longitude as double) from airports_str")
            
dbSendQuery(sc,"CREATE EXTERNAL TABLE IF NOT EXISTS airlines_bi_pq 
  ( year INT, month INT, day INT, dayofweek INT, dep_time INT, crs_dep_time INT, arr_time INT, 
    crs_arr_time INT, carrier STRING, flight_num INT, tail_num INT, actual_elapsed_time INT, crs_elapsed_time INT, 
    airtime INT, arrdelay INT, depdelay INT, origin STRING, dest STRING, distance INT, taxi_in INT, taxi_out INT, 
    cancelled INT, cancellation_code STRING, diverted INT, carrier_delay INT, weather_delay INT, nas_delay INT, 
    security_delay INT, late_aircraft_delay INT, date_yyyymm STRING) STORED AS PARQUET LOCATION '/tmp/airlines'")
